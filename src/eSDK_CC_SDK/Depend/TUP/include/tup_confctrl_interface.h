 ...
 *        <br>[cn]设置鉴权凭证(Token)(mediax组网 uportal组网)
 * 
 ...
 *            <br>[cn]此接口应用于VC解决方案IMS Hosted&SP Hosted组网、EC解决方案IMS Hosted&SP Hosted组网和PBX解决方案下的内置会议所有组网
 * @see NA
 **/
 ...
 *            <br>[cn]因SDK不存储账号密码，所以在每次使用接口tup_confctrl_book_conf预约会议前都要调用
 ...
 *        <br>[cn]预约会议(延时召开或立即召开)(MCU组网 mediax组网 uportal组网)
 * 
 ...
 *                                               <br>[cn]Hosted VC会议，对应的数据结构为CONFCTRL_S_BOOK_CONF_INFO_MEDIAX；
 ...
 *            <br>[cn]Convergent meeting 融合会议(包括PBX内置会议)，对应的回调事件为CONFCTRL_E_EVT_GET_CONF_LIST_RESULT
 ...
 *        <br>[cn]创建一个会控句柄(MCU组网 uportal组网)
 * 
 ...
 *            <br>[cn]当前仅在Convergent meeting 融合会议(包括PBX内置会议)下支持，对应的结果事件通知为CONFCTRL_E_EVT_LOCK_CONF_RESULT
 ...
 *        <br>[cn]发起两方转三方会议 (uportal组网)。UI已创建会议成功，TUP发REFER给SIP server，把呼叫对端拉入会议。
 * 
 ...
 *                                                      <br>[cn]观看多流与会者画面信息
 *                                                      <br>[cn]在On-premise VC会议下，对应的数据结构为CONFCTRL_S_WATCH_SVC_ATTENDEES；
