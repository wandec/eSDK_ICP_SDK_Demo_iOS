...
#define OFFLINE_D_MAX_FILENAME      255                             /**< [en]Indicates the maixmum length of file name. 
                                                                         <br>[cn]最长文件名长度 */
...
typedef enum tagOFFLINE_E_ERRORCODE{
    ...
    EN_OFFLINE_ERROR_NOHTTPRESPONSE,          /**< EN_OFFLINE_ERR_STATR+228: [en]Indicates that no http response. [cn]无http 响应*/
...
typedef struct tagOFFLINE_S_CIPHERLIST
{
    ...
    OFFLINE_E_CIPHER_TYPE *peCipherList;           /**[en]cipher list [cn]加密套内容*/
...
typedef struct tagST_OFFLINE_USERINFO
{
    ...
    TUP_CHAR*   pcPassword;                               /**< [en]Indicates user password ,required. [cn]用户密码，必填 */
    TUP_CHAR*   pcAppName;                                /**< [en]Indicates application name,optional. [cn]应用名称（如：eSpace），可选 */
...
{
    OFFLINE_CALLBACK_TYPE   enCallbackType;       /**< [en]Indicates the callback type of file transfer to upper, @see OFFLINE_CALLBACK_TYPE. [cn]文件传输上层回调类型, @see OFFLINE_CALLBACK_TYPE[TODO] */
