...
- (void)rotateClick
{
    ...
    BOOL success = [[CCSDK shareInstance] setVideoRotate:_rotate];
...
- (void)speakMute
{
    if (_isMute)
    {
        [[CCSDK shareInstance] setSpeakerMute:NO];
        [self.muteBtn setTitle:NSLocalizedString(@"Mute", "") forState:UIControlStateNormal];
        _isMute = NO;
        return;
    }
    [[CCSDK shareInstance] setSpeakerMute:YES];
...
- (void)TPCall
{
     [[CCSDK shareInstance] setTransportSecurityUseTLS:[LoginInfo sharedInstance].isTLS useSRTP:[LoginInfo sharedInstance].isTLS];
    [[CCSDK shareInstance] setDataRate:768];
    [[CCSDK shareInstance] setVideoMode:0];
    [[CCSDK shareInstance] setVideoContainer:self.localView remoteView:self.remoteView];
    [[CCSDK shareInstance] makeCall:[LoginInfo sharedInstance].TPACode callType:VIDEO_CALL callData:[LoginInfo sharedInstance].TPCallData verifyCode:[LoginInfo sharedInstance].VCode];
