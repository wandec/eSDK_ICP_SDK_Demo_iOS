...
@property (nonatomic, strong) MSEAGView *remoteView;
@property (nonatomic, strong) MSEAGView *localView;
...
- (void)callFail:(NSNotification *)notify
{
    ...
    if (_isWebCall)
    {
        [[CCSDK shareInstance] releaseWebChatCall];
        _isWebCall = NO;
          dispatch_async(dispatch_get_main_queue(), ^{
    ...
    dispatch_source_set_event_handler(_callTimer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                {
                    [[CCSDK shareInstance] releaseCall];
...
- (void)setSIPAddress{
    [[CCSDK shareInstance] setTransportSecurityUseTLS:NO useSRTP:NO];
    [[CCSDK shareInstance] setSIPServerAddress:[LoginInfo sharedInstance].MSSipIp port:[LoginInfo sharedInstance].MSSipPort];
}

- (void)setVideoView
{
    [[CCSDK shareInstance] setVideoContainer:self.localView remoteView:self.remoteView];
    [[CCSDK shareInstance] setDesktopShareContainer:self.screenshareView];
}

- (void)getQueueInfo
{
    [[CCSDK shareInstance] getCallQueueInfo];
}

- (void)doWebChatCall
{
    [[CCSDK shareInstance] webChatCall:[LoginInfo sharedInstance].MSChatACode callData:[NSString stringWithFormat:@"(CHAT)%@",[LoginInfo sharedInstance].MSCallData] verifyCode:[LoginInfo sharedInstance].VCode];
    _isWebCall = YES;
    [self startCallTimer];
...
- (void)cancelQueue
{
    [[CCSDK shareInstance] cancelQueue];
    [self stopQueueTimer];
    dispatch_async(dispatch_get_main_queue(), ^{
        [self.queueinfoText removeFromSuperview];
        [self.cancelQueueBtn removeFromSuperview];
...
- (void)videoMode
{
    if (_isHResolution)
    {
        [[CCSDK shareInstance] setVideoMode:1];
        _isHResolution = NO;
        return;
    }
    [[CCSDK shareInstance] setVideoMode:0];
    _isHResolution = YES;
}
...
- (void)sendMessageClick
{
    ...
    NSInteger ret = [[CCSDK shareInstance] sendMsg:self.chatText.text];
    if (ret != RET_OK)
    {
        [CCDemoUtil showAlertWithTitle:NSLocalizedString(@"Remind", "")
...
- (void)videoCallClick
{
    ...
    else
    {
        ...
        if(_audioSuccess)
        {
            ...
            [[CCSDK shareInstance] updateToVideo];
        }
        else
        {
            self.remindLabel.text = [NSString stringWithFormat:@"%@ %@",NSLocalizedString(@"Video", ""),NSLocalizedString(@"Calling", "")];
            [[CCSDK shareInstance] makeCall:[LoginInfo sharedInstance].MSAudioACode callType:VIDEO_CALL callData:[NSString stringWithFormat:@"(VIDEO CALL)%@",[LoginInfo sharedInstance].MSCallData] verifyCode:self.verifyText.text];
        }
    }
}
...
-(void)audioCall
{
    ...
    if (!_audioSuccess)
    {
        [[CCSDK shareInstance] makeCall:[LoginInfo sharedInstance].MSAudioACode callType:AUDIO_CALL callData:[NSString stringWithFormat:@"(AUDIO CALL)%@",[LoginInfo sharedInstance].MSCallData] verifyCode:self.verifyText.text];
    }
    else
    {
        [[CCSDK shareInstance] releaseCall];
...
- (void)rotateClick
{
    ...
    [[CCSDK shareInstance] setVideoRotate:_rotate];
}
...
- (void)composition
{
    ...
    self.remoteView = [MSEAGView getRemoteVideoViewWithFrame:CGRectMake(0, 0, _KWIDTH, kSHEIGHT - 2*ButtonHeight)];
    self.remoteView.backgroundColor = [UIColor blackColor];
    self.localView = [MSEAGView getLocalVideoViewWithFrame:CGRectMake(_KWIDTH - LocalViewWidth, 0, LocalViewWidth, LocalViewHeight)];
